...
# main molecules
starting_geoms = "trimer"
resulting_geoms = "tetramer"
namestringlist = []
...
for file in all_files:
    ...
    inner_ring_main_list = list(inner_ring_main_list)
    n_subs_per_ring = list(map(int, n_subs_per_ring))


    """print(inner_atom_main_list)
    inner_atom_main_list_idx = np.argsort(inner_atom_main_list1)
    print(inner_atom_main_list_idx)

    inner_atom_main_list = inner_atom_main_list1[inner_atom_main_list_idx].tolist()
    outer_atom_main_list = outer_atom_main_list[inner_atom_main_list_idx].tolist()
    inner_pos_main_list = inner_pos_main_list[inner_atom_main_list_idx].tolist()
    inner_ring_main_list = inner_ring_main_list[inner_atom_main_list_idx].tolist()"""
    ...
    """tmp_frag_atom = []
    for i in range(3):
        if len(frag_atom[i]) != 0:
            tmp_frag_atom.append(frag_atom[i][0])
        else:
            tmp_frag_atom.append(1000)

    tmp_arg = []
    for i in range(len(tmp_frag_atom)):
        if tmp_frag_atom[i] == 1000:
            tmp_arg.append()
        else:
            tmp_arg.append(inner_atom_main_list1.tolist().index(tmp_frag_atom[i]))"""

    #tmp_frag_atom = [frag_atom[0][0], frag_atom[1][0], frag_atom[2][0], frag_atom[3][0]]
            ...
            for idx in range(len(tmp_inner_list)):
                ...
                if symmetry == "nosym":
                    if substituted_ring == 0:
                        namestring = [subs, filestring1[:3], filestring1[4:7], filestring1[8:11], int(pos_dict[tmp_inner_list[idx]][0]), inner_pos_main_list[idx_main],
                                      int(filestring2[0]), int(filestring2[2]), int(filestring2[4]), int(filestring2[-1])]
                        if namestring not in namestringlist:
                            ...
                            result.close()
                        namestringlist.append([subs, filestring1[:3], filestring1[4:7], filestring1[8:11], int(pos_dict[tmp_inner_list[idx]][0]), inner_pos_main_list[idx_main],
                                                    int(filestring2[0]), int(filestring2[2]), int(filestring2[4]), int(filestring2[-1])])
                        namestringlist.append([filestring1[8:11], filestring1[4:7], filestring1[:3], subs, int(filestring2[-1]), int(filestring2[4]), int(filestring2[2]), int(filestring2[0]), inner_pos_main_list[idx_main], 
                                                    int(pos_dict[tmp_inner_list[idx]][0])])
                            
                    if substituted_ring == 1:
                        print(filestring1)
                        print(filestring2)
                        namestring = [filestring1[:3], filestring1[4:7], filestring1[8:11], subs, int(filestring2[0]), int(filestring2[2]), int(filestring2[4]), int(filestring2[-1]),
                                      inner_pos_main_list[idx_main], int(pos_dict[tmp_inner_list[idx]][0]), "branch"]
                        if namestring not in namestringlist:
                            result = open(resulting_geoms + "/" + filestring1 +"-"+ subs + "_" + filestring2 +"-"
                                    + str(inner_pos_main_list[idx_main]) + "-"
                                    + str(pos_dict[tmp_inner_list[idx]][0])  +'_branch.xyz', 'w+')
                            ...
                            result.close()
                        namestringlist.append([filestring1[:3], filestring1[4:7], filestring1[8:11], subs, int(filestring2[0]), int(filestring2[2]), int(filestring2[4]), int(filestring2[-1]),
                                                inner_pos_main_list[idx_main], int(pos_dict[tmp_inner_list[idx]][0]), "branch"])
                        namestringlist.append([filestring1[8:11], filestring1[4:7], filestring1[:3], subs, int(filestring2[-1]), int(filestring2[4]), int(filestring2[2]), int(filestring2[0]), 
                                               inner_pos_main_list[idx_main], int(pos_dict[tmp_inner_list[idx]][0]), "branch"])
                        namestringlist.append([subs, filestring1[4:7], filestring1[:3], filestring1[8:11], int(pos_dict[tmp_inner_list[idx]][0]), inner_pos_main_list[idx_main], 
                                               int(filestring2[2]), int(filestring2[0]), int(filestring2[4]), int(filestring2[-1]), "branch"])
                        namestringlist.append([filestring1[8:11], filestring1[4:7], filestring1[:3], subs, int(filestring2[-1]), int(filestring2[4]), int(filestring2[2]), int(filestring2[0]), 
                                               inner_pos_main_list[idx_main], int(pos_dict[tmp_inner_list[idx]][0]), "branch"])


                    if substituted_ring == 2:
                        namestring = [filestring1[:3], filestring1[4:7], filestring1[8:11], subs, int(filestring2[0]), int(filestring2[2]), int(filestring2[4]), int(filestring2[-1]),
                        ...
                        namestringlist.append([filestring1[:3], filestring1[4:7], filestring1[8:11], subs, int(filestring2[0]), int(filestring2[2]), int(filestring2[4]), int(filestring2[-1]),
                                                inner_pos_main_list[idx_main], int(pos_dict[tmp_inner_list[idx]][0])])
                        namestringlist.append([subs, filestring1[8:11], filestring1[4:7], filestring1[:3], int(pos_dict[tmp_inner_list[idx]][0]), inner_pos_main_list[idx_main],
                                                int(filestring2[-1]), int(filestring2[4]), int(filestring2[2]), int(filestring2[0])])
                            
                elif symmetry == "sym":
                    namestring = [filestring1[:3], filestring1[3:6], subs, int(filestring2[0]), int(filestring2[-1]),
                                  inner_pos_main_list[idx_main], int(pos_dict[tmp_inner_list[idx]][0])]
                    if namestring not in namestringlist:
                        result = open(resulting_geoms + "/" + filestring1 +"-"+ subs + "_" + filestring2 +"-"
                                    + str(inner_pos_main_list[idx_main]) + "-"
                        ...
                        result.close()                        
                        namestringlist.append([filestring1[:3], filestring1[3:6], subs, int(filestring2[0]), int(filestring2[-1]),
                                            inner_pos_main_list[idx_main], int(pos_dict[tmp_inner_list[idx]][0])])
                        namestringlist.append([subs, filestring1[3:6], filestring1[:3], int(pos_dict[tmp_inner_list[idx]][0]),
