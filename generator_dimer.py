...
# main molecules
starting_geoms = "monomer"
resulting_geoms = "dimer"
namestringlist = []
...
for file in all_files:
    ...
    n_subs_per_ring = [i for i in avail_pos[int(3*tmp_len):]]
    ...
    for idx_main in range(len(inner_atom_main_list)):
        ...
        for subs in ["DHI", "DKI", "MKI"]:
            ...
            #print("tmp_inner_list",tmp_inner_list)
            for idx in range(len(tmp_inner_list)):
                ...
                comment_line1 = ' '.join(str(remaining_atom_main_inner[i])+str(remaining_ring_main_inner[i]) 
                                         for i in range(len(remaining_atom_main_inner)))
                comment_line2 = ' '.join(str(i) for i in remaining_atom_main_outer)
                comment_line3 = ' '.join(str(i) for i in remaining_pos_main_inner)

                comment_line4 = ' '.join(str(remaining_atom_subs_inner[i])+str(remaining_ring_subs_inner[i]) 
                                         for i in range(len(remaining_ring_subs_inner)))
                ...
                namestring = [filestring1, subs, int(pos_dict[tmp_inner_list[idx]][0]), inner_pos_main_list[idx_main]]

                if (filestring1 == subs) and (int(pos_dict[tmp_inner_list[idx]][0]) == inner_pos_main_list[idx_main]):
                    comment_line8 = 'sym'
                else:
                    comment_line8 = 'nosym'
                ...
                if namestring not in namestringlist:
                    ...
                    result = open(resulting_geoms + "/" + filestring1 + "-" + subs + "_" 
                                  + filestring2 + str(inner_pos_main_list[idx_main]) + "-"
                    ...
                    namestringlist.append([filestring1, subs, 
                                           int(pos_dict[tmp_inner_list[idx]][0]), inner_pos_main_list[idx_main]])
                    namestringlist.append([subs, filestring1, 
                                           inner_pos_main_list[idx_main], int(pos_dict[tmp_inner_list[idx]][0])])
                    
                    print([filestring1, subs, int(pos_dict[tmp_inner_list[idx]][0]), inner_pos_main_list[idx_main]])
                    print([subs, filestring1, inner_pos_main_list[idx_main], int(pos_dict[tmp_inner_list[idx]][0])])
